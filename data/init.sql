...
CREATE TABLE c2 (
  ...
  `id_c1` int UNSIGNED NOT NULL,
  `state` tinyint UNSIGNED NULL,
  `created_at` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp(0) NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '更新时间',
  CONSTRAINT `c2_fk_c1` FOREIGN KEY (`id_c1`) REFERENCES c1 (`id`) ON DELETE RESTRICT ON UPDATE CASCADE
) CHARACTER SET = utf8mb4;
-- 创建图书表
CREATE TABLE book (
  `id` int UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `created_at` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp(0) NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '更新时间',
  CONSTRAINT `book_fk_c1` FOREIGN KEY (`id_c1`) REFERENCES c1 (`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `book_fk_c2` FOREIGN KEY (`id_c2`) REFERENCES c2 (`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `book_fk_author` FOREIGN KEY (`id_author`) REFERENCES author (`id`) ON DELETE RESTRICT ON UPDATE CASCADE
) CHARACTER SET = utf8mb4;
-- 创建用户表
CREATE TABLE user(
